...
rd.fun <- function(ooclat, quota) {
  ...
  for (i in 2:(nrow(xts)-1)) {
    ...
    if (cns %in% colnames(xocc.2a ) == TRUE) {
      ...
      md2a[i-1,5] <- rarx[1]+(1.96*rarx[2])
      md2a[i-1,6] <- rarx[1]-(1.96*rarx[2])
    }
  }
  bin.name <- bin.name[-1]
...
{ 
  ...
  for (i in 1:4) ### ### loop for each latitudinal zone
  {
    ...
    for (k in 2:(nrow(xts)-1)) ## loop for each time slice
    {
      ...
      rp[5,k-1,i] <- 1.96*as.numeric(as.character(gamma.d[2])) # CI
      rp[6,k-1,i] <- as.numeric(as.character(beta.d[1]))
      rp[7,k-1,i] <- 1.95*as.numeric(as.character(beta.d[2])) # CI
      rp[8,k-1,i] <- as.numeric(as.character(alpha.d[1]))
      rp[9,k-1,i] <- 1.95*as.numeric(as.character(alpha.d[2])) # CI
    }
  }
  return(rp)
}
...
{ 
  ...
  for (i in 2:(nrow(xts)-1)) ### loop for each time slice !! avoid i=5 (nrow(xts)-1)
  {
    ...
    for (k in 1:4) ### loop for each latitudinal zone
    {
      ...
      rk[5,k,i-1] <- 1.95*as.numeric(as.character(gamma.d[2])) # SE
      rk[6,k,i-1] <- as.numeric(as.character(beta.d[1]))
      rk[7,k,i-1] <- 1.95*as.numeric(as.character(beta.d[2])) # SE
      rk[8,k,i-1] <- as.numeric(as.character(alpha.d[1]))
      rk[9,k,i-1] <- 1.95*as.numeric(as.character(alpha.d[2])) # SE
    }
  }
  return(rk)
}
