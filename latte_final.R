...
for (k in 2:(nrow(xts)-1)) 
{
  ...
  r.gam[5,k-1] <- as.numeric(as.character(gamma.d[2])) # SE
  r.gam[6,k-1] <- as.numeric(as.character(beta.d[1]))
  r.gam[7,k-1] <- as.numeric(as.character(beta.d[2])) # SE
  r.gam[8,k-1] <- as.numeric(as.character(alpha.d[1]))
  r.gam[9,k-1] <- as.numeric(as.character(alpha.d[2])) # SE
}
...
trend.fun <- function(ipua, ipub) {
  ...
  pear.c <- cor(cbind(malpha.a, malpha.b), method="pearson")
  spear.c <- cor(cbind(malpha.a, malpha.b), method="spearman")
  resis <- rbind(pear.c, spear.c)
  return(resis)
}
